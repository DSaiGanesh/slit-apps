...
st.image(r'C:\Users\s4nto\OneDrive - St. Clair College\GroupActivitiesSemester4\Financial Analytics\Final Project\stock_market.jpg')

value = "GOOG"
ticker = st.sidebar.text_input('Name of Stock', value=value).upper()
...
selected_duration = st.sidebar.selectbox('Select Duration', ["1 Year", "5 Years", "10 Years", "Custom Date"])
...
if selected_duration == "Custom Date":
    start_date = st.sidebar.date_input("Select Start Date", value=datetime.date.today() - datetime.timedelta(days=365), min_value=datetime.date(1900, 1, 1), max_value=datetime.date.today())
...
Introduction, Financial_Ratio, Monte_Carlo_Simulation, Predictions, GoldenCross = st.tabs(["Overview","Financial Ratios","Monte Carlo Simulation", "Stock Price Predictions", "Golden Cross"])
...
with Introduction:
    ...
    st.plotly_chart(fig)
    with st.expander(f"{ticker} stock data details"):
        data2 = data.copy()
